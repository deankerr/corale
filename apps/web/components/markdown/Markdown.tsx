...
const components = {
  ...
  h2: ({ node, color, ...props }: WithExtraProps<'h2'>) => <Text as="div" size="5" weight="bold" mt="3" {...props} />,
  h3: ({ node, color, ...props }: WithExtraProps<'h3'>) => <Text as="div" size="4" weight="medium" mt="2" {...props} />,
...
export const Markdown = memo(({ children }: { children?: string | null | undefined }) => {
  return (
    <ReactMarkdown remarkPlugins={[remarkGfm, remarkBreaks]} components={components} className="markdown-root">
      {children}
    </ReactMarkdown>
  )
})
