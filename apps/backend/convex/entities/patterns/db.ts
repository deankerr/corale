...
export async function getPattern(ctx: QueryCtx, { patternId }: { patternId: string }) {
  ...
  const pattern = docId
    ? await ctx.table('patterns').get(docId)
    : await ctx.table('patterns').get('xid', patternId)
...
export async function getPatternX(ctx: QueryCtx, { patternId }: { patternId: string }) {
  ...
  if (!pattern || pattern.deletionTime)
    throw new ConvexError({ message: `Invalid pattern id`, id: patternId })
  return pattern
}
